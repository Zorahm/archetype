...
Платформо-независимое ядро. ~95% всего кода. Компилируется с fabric-loader (для маппингов) и architectury API.

## Точка входа

`Archetype.init()` — вызывается из fabric/forge инициализаторов. Регистрирует builtin условия, способности, advancement triggers, сеть.
